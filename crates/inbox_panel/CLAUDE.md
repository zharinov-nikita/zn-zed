...
- When a `WorktreeRemoved` event arrives, the worktree is already gone from the project, so `inbox_abs_path` returns `None`. `rebind_worktree` flushes unsaved edits to the outgoing file through the cached `bound_paths` — don't "simplify" that into a live path lookup.
